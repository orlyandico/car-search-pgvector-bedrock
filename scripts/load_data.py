...
def load_data(conn, df, batch_size):
    ...
    with conn.cursor() as cur:
        ...
        for i in range(0, total, batch_size):
            ...
            print(f"Processed {min(i+batch_size, total)}/{total} rows ({batch_elapsed:.1f}s)")

def main():
    parser = argparse.ArgumentParser()
    parser.add_argument('--batch-size', type=int, default=1000)
    parser.add_argument('--truncate', action='store_true')
    parser.add_argument('--info', action='store_true', help='Show data info without loading')
    ...
    price_filtered = df[(df['price'].isna()) | ((df['price'] >= price_lower) & (df['price'] <= 250000))].copy()
    print(f"Price filter removed {len(df) - len(price_filtered)} rows")
    df = price_filtered
    
    year_filtered = df[(df['year'].isna()) | ((df['year'] >= year_lower) & (df['year'] <= 2030))].copy()
    ...
    text_cols = ['region', 'manufacturer', 'model', 'condition', 'cylinders', 'fuel', 
